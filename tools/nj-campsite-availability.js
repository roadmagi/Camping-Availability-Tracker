...
// Returns { parkName, sites: [{ siteId, shortName, name, type, cost, days:{iso:status} }] }
async function getParkAvailability(park, startDate, months) {
  const session = await getSession(park.id);
  const endDate = (() => {
    const e = new Date(Date.UTC(startDate.getUTCFullYear(), startDate.getUTCMonth() + months, startDate.getUTCDate()));
    return e;
  })();
  ...
  while (cursor < endDate && guard < 60) {
    guard++;
    const sites = await fetchWindow(session, park.id, cursor);
    for (const s of sites) {
      ...
      for (const dd of s.Dates || []) {
        ...
        if (!dt) continue;
        entry.days[isoUTC(dt)] = dayStatus(dd);
      }
    }
